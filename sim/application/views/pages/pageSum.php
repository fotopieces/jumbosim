...
function newMenu(){
	...
	window.location="<?=url::base()?>adminCtrl/newSum/0";
	
}
function doEditMenu(id){
	//alert(1);
	window.location="<?=url::base()?>adminCtrl/newSum/"+id;
...
function dodelete(id){
	if(confirm("ต้องการลบข้อมูลหรือไม่")){
		window.location="<?=url::base()?>adminCtrl/delSum/"+id;
	}
}
...
<!-- 			ค้นหา<small> เบอร์สวย เบอร์มงคล</small> -->
		...
		<div class="alert alert-dismissable ">
			...
			<button  class="btn btn-success" type="button" onclick="newMenu()"><div id="btname">เพิ่มผลรวม</div>  </button>
		</div>
	</div>
</div>
<div class="row"></div>
		...
		<div class="panel panel-primary">
			<div class="panel-heading">
				<h3 class="panel-title">
					<i class="fa fa-tasks"></i> ผลรวม
				</h3>
			</div>
			<div class="panel-body">
				<table style="width: 100%" border="1">
<?php 	$listCatalogsim=ORM::factory("pagesim")->where("status","S")->find_all();
$i=1;
